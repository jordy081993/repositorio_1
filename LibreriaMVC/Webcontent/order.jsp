...
<body>
	...
	<form name="AnyadirForm" action="shopping" method="POST">
		<input type="hidden" name="todo" value="add"> T´ıtulo: <select
			name=idLibro>
			<%
			...
			for (int i = 0; i < LibrosBD.tamanyo(); i++) {
				...
				+ LibrosBD.getCantidad(i));
				out.println("</option>");
			}
			%>
		</select> Cantidad: <input type="text" name="cantidad" size="10" value="1"><input
			type="submit" value="A~nadir a la cesta">
	</form>
	<br />
	<hr />
	<br />
	...
	<table border="1" cellspacing="0" cellpadding="5">
		...
		for (int i = 0; i < cesta.size(); i++) {
			...
			for(int j=0;j<LibrosBD.tamanyo();j++){
				if(elementoPedido.getTitulo().equals(LibrosBD.getTitulo(j))){
					x=j;
				}
			}
			if (LibrosBD.getCantidad(x) >= elementoPedido.getCantidad()) {
		%>
		<tr>
